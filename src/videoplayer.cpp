...
VideoPlayer::~VideoPlayer() {
    ...
    if (osdLabel) { delete osdLabel; osdLabel = nullptr; }
...
{
    setWindowIcon(QIcon(":/icons/MediaPlayer3.png"));
...
void VideoPlayer::setupMenu()
{
    ...
    // 关于
    contextMenu->addSeparator();
    QAction *aboutAction = contextMenu->addAction("关于 MediaPlayer3");
    connect(aboutAction, &QAction::triggered, this, &VideoPlayer::showAboutDialog);
...
void VideoPlayer::showAboutDialog()
{
    QDialog dlg(this);
    dlg.setWindowTitle("关于 MediaPlayer3");
    ...
    QPixmap iconPixmap(":/icons/MediaPlayer3.png");
    if (!iconPixmap.isNull()) {
        iconPixmap = iconPixmap.scaled(72, 72, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        iconLabel->setPixmap(iconPixmap);      //
    } else {
        iconLabel->setText("?");               // 兜底
    }

    QVBoxLayout *titleLayout = new QVBoxLayout();
    titleLayout->setSpacing(2);

    QLabel *nameLabel = new QLabel("MediaPlayer3", &dlg);
    QFont nameFont = nameLabel->font();
    nameFont.setPointSize(16);
    nameFont.setBold(true);
    nameLabel->setFont(nameFont);

    QLabel *verLabel = new QLabel("版本 1.0.0", &dlg);
    ...
    infoLabel->setStyleSheet("font-size: 12px; line-height: 1.6;");
    infoLabel->setText(QString(
                           "<b>作者：</b>berylok<br>"
                           "<b>构建：</b>Qt %1<br>"
                           "<b>许可：</b>MIT License<br>"
                           "<b>系统：</b>%2"
                           ).arg(QT_VERSION_STR, QSysInfo::prettyProductName()));
